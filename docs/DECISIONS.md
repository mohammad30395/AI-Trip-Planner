...
- Use Wikimedia Commons as the current secondary image provider for
  representative destination covers and accepted exact places, including hotels,
  only through strict exact-place title matching. Ambiguous or unrelated hotel
  media must keep the neutral image fallback.
